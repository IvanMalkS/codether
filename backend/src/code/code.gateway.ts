...
export class CodeGateway {
    ...
    async handleJoinCodeRoom(
        ...
        try {
            ...
            const code = await this.codeService.findOne(codeId, { viewPassword } as FindCodeDto);
    ...
    async handleEditCode(@MessageBody() UpdateSocketCodeDto: UpdateSocketCodeDto, @ConnectedSocket() client: Socket) {
        const { id, updatePassword, code } = UpdateSocketCodeDto;
        try {
            // Validate the password and update the code
            const updatedCode = await this.codeService.update(id, { updatePassword, code });
